...
public class WordLadder {
	...
	private static boolean processNeighbors(QNode x) {
		...
		for (int i = 0; i < word.length(); ++i) {
			...
			for (char z = 'a'; z <= 'z'; ++z) {
				...
				if (temp2.equals(end)) { // if altering the word finds the result, return
					R.insert(end, word);
					return true;
				}
				if (R.find(temp2) == null && T.find(temp2) != null) { // else  continue processing
	...
	public static void main(String [] args) throws IOException {
		...
		boolean found = false;
		if (start.equals(end)) // base case, the word is the same.
			found = true;

		while (!Q.isEmpty() && !found) { // while the Q is not empty, and we haven't found the word.
			QNode x = Q.dequeue(); // pop off the queue.
			if (processNeighbors(x)) // process this word
				found = true;
		}
